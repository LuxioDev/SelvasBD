...
def exportar_a_excel(tree):
    ...
    if os.path.exists(filename):
        ...
        headers = ["Fecha", "Usuario", "Sucursal", "Producto", "Cantidad", "Precio Unitario", "Total Ajustado", "Cupon Usado", "Metodo de Pago"]
    ...
    for item in tree.get_children():
        ...
        producto, fecha_movimiento, cantidad_str, total_str, total_ajustado_str, cupon_usado, metodo_pago = values

        # Convertir cantidad y total a flotantes
        cantidad = float(cantidad_str) if cantidad_str else 0
        total_ajustado = float(total_ajustado_str) if total_ajustado_str else 0
        precio_unitario = total_ajustado / cantidad if cantidad > 0 else 0  # Cálculo del precio unitario

        # Obtener la fecha actual para la exportación
        fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Agregar fila al Excel
        fila = [fecha, id_usuario, sucursal, producto, cantidad, precio_unitario, total_ajustado, cupon_usado, metodo_pago]
        sheet.append(fila)
        ...
        try:
            ...
            consulta = """
                SELECT m.id_movimiento, p.descripcion, DATE_FORMAT(m.fecha, '%d/%m') as fecha_movimiento,
                       ...
                       (CASE 
                           WHEN m.metodo_pago = 'efectivo' THEN (m.cantidad * p.precio) * 0.85
                           WHEN m.metodo_pago = 'tarjeta' THEN (m.cantidad * p.precio) * 1.15
                           ELSE (m.cantidad * p.precio)
                       END) as total_ajustado,
                       m.cupon_usado, m.metodo_pago
            ...
            for movimiento in movimientos:
                tree.insert("", "end", iid=movimiento[0], values=movimiento[1:])  # Asegúrate de que los valores tengan el nuevo total ajustado
