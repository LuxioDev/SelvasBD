        ...
        try:
            ...
            if resultado:
                ...
                if stock_actual + cantidad_anadir >= 0:
                    ...
                    # Aquí solo obtenemos el ID del producto y la sucursal
                    cursor.execute(
                    ...
                    # Ya no llamamos a registrar_movimiento aquí
                    conexion.commit()
                    return True
                else:
...
def registrar_compra_con_descuento(id_producto, id_sucursal, id_usuario, tipo_movimiento, cantidad, descripcion, metodo_pago, cupon_usado):
    conexion = conectar_bd()
    if conexion:
        cursor = conexion.cursor()
        try:
            consulta = """
                INSERT INTO historial_movimientos (id_producto, id_sucursal, id_usuario, tipo_movimiento, cantidad, descripcion, metodo_pago, cupon_usado)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """
            cursor.execute(consulta, (id_producto, id_sucursal, id_usuario, tipo_movimiento, cantidad, descripcion, metodo_pago, cupon_usado))
            conexion.commit()
        except mysql.connector.Error as err:
            CTkMessagebox(title="Error", message=f"No se pudo registrar el movimiento: {err}")
        finally:
...
def obtener_id_producto(descripcion_producto):
    conexion = conectar_bd()
    if conexion:
        cursor = conexion.cursor()
        try:
            consulta = "SELECT id_producto FROM productos WHERE descripcion = %s"
            cursor.execute(consulta, (descripcion_producto,))
            resultado = cursor.fetchone()
            if resultado:
                return resultado[0]
            else:
            ...
            print(f"Error al obtener el ID del producto: {err}")
        finally:
            cursor.close()
            conexion.close()
    return None
    ...
    if selected_item:
        ...
        try:
            ...
            if cantidad < 0:
                ...
                monto_total = abs(cantidad) * precio_unitario  # Total sin ajuste
                cantidad_productos = abs(cantidad)
                sucursal = usuario_actual.usuario_actual[3]

                codigo_cupon = cupon_entry.get().strip()
                descuento_cupon = verificar_cupon(codigo_cupon) or 0

                # Inicializar total con el monto total
                total_con_descuento = monto_total

                # Aplicar 15% de descuento si el método de pago es efectivo
                if metodo_pago_var.get() == "efectivo":
                    total_con_descuento *= 0.85  # Aplicar el 15% de descuento
                # Aplicar 15% de aumento si el método de pago es tarjeta
                elif metodo_pago_var.get() == "tarjeta":
                    total_con_descuento *= 1.15  # Aumentar el 15%

                # Obtener el ID del producto
                id_producto = obtener_id_producto(producto)
                if id_producto is None:
                    CTkMessagebox(title="Error", message="El producto no se encontró en la base de datos.")
                    return

                # Ahora registramos el método de pago y si se usó un cupón
                metodo_pago = "efectivo" if metodo_pago_var.get() == "efectivo" else "tarjeta"
                cupon_usado = codigo_cupon if descuento_cupon > 0 else None

                exito = modificar_stock_producto(producto, cantidad)

                if exito:
                    # Registrar el movimiento con el total después del ajuste
                    registrar_compra_con_descuento(id_producto, sucursal, id_usuario, 'salida', abs(cantidad), f"Venta del producto con ajuste de precio. Total: ${total_con_descuento:.2f}", metodo_pago, cupon_usado)
